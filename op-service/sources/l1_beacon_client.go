...
func (cl *BeaconHTTPClient) BeaconBlobSideCars(ctx context.Context, fetchAllSidecars bool, slot uint64, hashes []eth.IndexedBlobHash) (eth.APIGetBlobSidecarsResponse, error) {
	...
	if len(indices) > 0 {
		return eth.APIGetBlobSidecarsResponse{}, fmt.Errorf("%s/%s?%s #returned blobs(%d) != #requested blobs(%d)", cl.cl.Endpoint(), reqPath, reqQuery.Encode(), len(hashes)-len(indices), len(hashes))
	}
	return resp, nil
}
